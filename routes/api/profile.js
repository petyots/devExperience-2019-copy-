...
router.post( "/", passport.authenticate("jwt", { session: false }),
  (req, res) => {
    ...
    // Get fields
    const profileFields = {};//all field from form
...
router.get('/handle/:handle', (req, res) => {
  ...
  Profile.findOne({ handle: req.params.handle })
    ...
    .catch(err => res.status(404).json({ profile: 'There is no profile for this handler' }));
});
...
router.post('/experience',passport.authenticate('jwt', { session: false }),
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Add to exp array
      profile.experience.unshift(newExp);//add to begin of array experience
...
router.post( '/education', passport.authenticate('jwt', { session: false }),
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Add to exp array add at beginning
